...
async def detection_loop():
    ...
    # Warm-up period: collect 5 rounds of data before scoring
    warmup_data = {svc: [] for svc in SERVICES}
    warmup_rounds = 0
        ...
        try:
            ...
            for service in SERVICES:
                try:
                    ...
                    if warmup_rounds < 5:
                        warmup_data[service].append(features)
                        continue

                    # Train on first run after warmup
                    if warmup_rounds == 5:
                        all_features = warmup_data[service]
                        if all_features:
                            feature_matrix = np.array([list(f.values()) for f in all_features])
                            isoforest.fit(feature_matrix)
                            lstm.fit(feature_matrix)

                    # 3. Score with both models
                    feature_vec = np.array(list(features.values())).reshape(1, -1)

                    iso_score = isoforest.score(feature_vec)
                    lstm_score = lstm.score(feature_vec)
